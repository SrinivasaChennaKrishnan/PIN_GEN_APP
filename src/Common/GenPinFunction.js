...
export const duplicateAndSequenceCheck = input => {
  ...
  if (convertedInput.length >= 2) {
    ...
    if (
      (digitOne === digitTwo - 1 && digitTwo === digitThree - 1) ||
      (digitTwo === digitThree - 1 && digitThree === digitFour - 1) ||
      (digitOne === digitTwo - 1 &&
        digitTwo === digitThree - 1 &&
        digitThree === digitFour - 1)
    ) {
      isAscending = true;
    }
...
export const GenPinFunction = () => {
  ...
  while (i <= requiredLen) {
    ...
    if (randomPin && randomPin.toString().length === requiredLen) {
      // push the pin if the length is 4
      newPin = randomPin;
    } else if (randomPin && randomPin.toString().length === requiredLen - 1) {
    ...
    if (
      !duplicateAndSequenceCheck(updatedPinseq) &&
      !pinArray.includes(updatedPinseq)
    ) {
    ...
    if (firstArrayVal) {
      for (let i = 0; i < pinArray.length; i++) {
        ...
        if (firstArrayVal !== pinArray[i + 1]) {
          ...
          // if similar to other values, generate new one and then push
          firstArrayVal = Math.floor(Math.random() * 10000);
          if (
            !duplicateAndSequenceCheck(firstArrayVal) &&
            !pinArray.includes(firstArrayVal)
          ) {
